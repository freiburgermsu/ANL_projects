...
class BaseFBAPkg:
    ...
    def build_variable(self, kind, lower_bound, upper_bound, vartype, object, object_type = 'reaction'):
        ...
        if object_type == 'reaction':
            for metabolite in object.metabolites:
                ...
                self.thermo_compounds[tmfa_name] = object.metabolites[metabolite]
        elif object_type == 'metabolite':
            tmfa_name = re.sub('(_\w$)', '', object.id)
    ...
    def build_constraint(self, constraint_expression, kind, lower_bound, upper_bound, coef, object, object_type = 'reaction'):
        ...
        if name in self.constraints[kind]:
            print('ERROR: A {} constraint already exists, {}.'.format(name, self.constraints[kind][name]))
        else:
...
class RevBinPkg(BaseFBAPkg):
    ...
    def build_constraint(self, object, object_type = 'reaction'):
        ...
        if object_type == 'reaction':
            ...
            if object.id not in self.variables['revbin']:
                BaseFBAPkg.build_variable(self, kind = "revbin", lower_bound = 0, upper_bound = 1, vartype = "binary", object = object, object_type = 'reaction')
...
class SimpleThermoPkg(RevBinPkg):
    def __init__(self, model, object, thermo_reactions, thermo_compounds):
        '''Redefining the inherited __init__ function
        'model' (COBRA object): The COBRApy FBA model
    ...
    def build_constraint(self, object, object_type = 'reaction'):
        ...
        BaseFBAPkg.build_variable(self, kind = "potential", lower_bound = 0, upper_bound = 1000, vartype = "continuous", object = object)
        ...
        if object_type == 'reaction':
            if object.id not in self.constraints['simple_thermo']:
                ...
                for metabolite in object.metabolites: 
                    self.variables["potential"][metabolite.id] = metabolite.id
                    stoichiometry = object.metabolites[metabolite]
                    coef[self.variables["potential"][metabolite.id]] = stoichiometry
                    
                BaseFBAPkg.build_constraint(self, constraint_expression = Zero, kind = "simple_thermo", lower_bound = 0, upper_bound = 1000, coef = coef, object = object, object_type = 'reaction')
            
            elif object.id in self.constraints['simple_thermo']:
                print('ERROR: A {} constraint already exists, {}.'.format(constraint_name, self.constraints[kind][object.id]))
    ...
    def build_package(self, filter = None):
        ...
        for metabolite in self.model.metabolites:
            BaseFBAPkg.build_variable(self, kind = "potential", lower_bound = 0, upper_bound = 1000, vartype = "continuous", object = metabolite, object_type = 'metabolite')
        
        for reaction in self.model.reactions: 
            stoichiometry = object.metabolites[metabolite.id]
            coef[self.variables["potential"][metabolite.id]] = stoichiometry
...
class FullThermoPkg(SimpleThermoPkg):
    def __init__(self, model, object, thermo_reactions, thermo_compounds, thermodynamics_data_type, object_type = 'reaction'):
        ...
        BaseFBAPkg.__init__(self, model = model, name = "full_thermo", variable_types = {"lnconc":"metabolite", 'thermo': 'reactions', "revbin":"reaction"}, constraint_types = {'concentration_potential': 'metabolite', 'electrochemical_potential':'metabolite', 'thermo': 'metabolite'})    
    ...
    def build_constraint(self, model, object, kind = 'full_thermo'): #, modelseed = modelseed):
        '''Build constraints through the inherited function and the calculated variable coeffiients 
        'object' (Python object, string): The variable name when the name is defined
        ...
        if constraint_name not in constraint_names:
            ...
            built_constraint = BaseFBAPkg.build_constraint(self, constraint_expression = Zero, kind = kind, lower_bound = constant, upper_bound = constant, coef = coef, object = object, object_type = 'reaction')
    ...
    def build_package(self):
        ...
        for metabolite in self.model.metabolites:
            BaseFBAPkg.build_variable(self, kind = "full_thermo", lower_bound = 0, upper_bound = 1000, vartype = "continuous", object = metabolite)
