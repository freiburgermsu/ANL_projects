    ...
    def build_constraint(self, obj, obj_type = 'reaction', view_errors = True, errors_quantity = 0, kind = 'simple_thermo'):
        ...
        if obj_type == 'reaction':
            if obj.id not in self.constraints['simple_thermo']:
                ...
                for metabolite in obj.metabolites: 
                    errors_quantity, self.variables["potential"][metabolite.id] = BaseFBAPkg.build_variable(self, kind = "potential", lower_bound = 0, upper_bound = 0, vartype = "continuous", obj = metabolite, obj_type = 'metabolite', view_errors = view_errors, errors_quantity = errors_quantity)
...
class FullThermoPkg(SimpleThermoPkg):
    def __init__(self, model, thermo_compounds, thermodynamics_data_type, obj_type = 'reaction'):
        ...
        BaseFBAPkg.__init__(self, model = model, name = "full_thermo", variable_types = {"revbin":"reaction", 'concentration': 'metabolite'}, constraint_types = {'concentration_potential': 'metabolite', 'electrochemical_potential':'metabolite', 'thermo': 'metabolite'})    
        
        # inherit the RevBinPkg variables and constraints
        previous_variables = {"revbin":"reaction", "forv":"reaction", "revv":"reaction"}
    ...
    def build_data(self, obj, view_errors = True, obj_type = 'reaction', errors_quantity = 0):
            ...
            for metabolite in obj.metabolites:
                ...
                if tmfa_name not in undescribed_compounds:          
                    ...
                    if tmfa_name not in self.parameters['custom_concentration_constraints']:
                        ...
                        activity_coefficient = self.parameters['activity_coefficient'][tmfa]
                    ...
                    errors_quantity, self.variables['concentration'][metabolite.id] = BaseFBAPkg.build_variable(self, kind = "concentration", lower_bound = concentrations[0], upper_bound = concentrations[1], vartype = "continuous", obj = metabolite, view_errors = view_errors, errors_quantity = errors_quantity, obj_type = 'metabolite')
                    
                    self.parameters['concentration_potential'][metabolite.id] = self.thermo_compounds[tmfa_name]['stoichiometry'] * ln(self.variables['concentration'][metabolite.id] * activity_coefficient)
    ...
    def build_constraint(self, obj, coef = {}, view_errors = True, errors_quantity = 0, kind = 'full_thermo', obj_type = 'reaction'): #, modelseed = modelseed):
        '''Build constraints through the inherited function and the calculated variable coeffiients 
        'obj' (Python obj, string): The variable name when the name is defined
        ...
        undescribed_compounds = self.build_data(view_errors = view_errors, obj = obj, obj_type = obj_type, errors_quantity = errors_quantity) 
        ...
        if self.thermo_data_type == 'dictionary':
            for metabolite in obj.metabolites:
                ...
                tmfa_name = re.sub('(?i)(_[a-z]$)', '', metabolite.id)
                if tmfa_name not in undescribed_compounds:
                    thermo += self.thermo_compounds[tmfa_name]['gibbs']    
        ...
        self.variables['total_energy'][obj.id] = delta_g + R * temperature * sum_concentration_potential + sum_electro_potential  
        constant = self.variables['total_energy'][obj.id]
        constraint_name = '{}_{}'.format(obj.id, kind)
        if constraint_name not in self.model.constraints:
            RevBinPkg.build_constraint(self, obj = obj, obj_type = 'reaction')
            coef[self.variables["revbin"][obj.id]] = 1000
            built_constraint = BaseFBAPkg.build_constraint(self, constraint_expression = Zero, kind = kind, lower_bound = constant, upper_bound = constant, coef = coef, obj = obj, view_errors = view_errors, obj_type = 'reaction')
        else:
            built_constraint = None
            if obj_type == 'reaction':
                obj.upper_bound = obj.lower_bound = constant
                print('The {} contraint is updated in the model: {}, ub: {}, lb: {}'.format(constraint_name, model.constraints[constraint_name], obj.upper_bound, obj.lower_bound))
            
        return built_constraint

        
    def build_package(self, filter = 'default_constraints', kind = "concentration", view_errors = True, errors_quantity = 0):
        ...
        for reaction in self.model.reactions: 
            if reaction.id not in filter:
                self.build_constraint(obj = reaction, obj_type = 'reaction', kind = kind, view_errors = view_errors, errors_quantity = errors_quantity)
